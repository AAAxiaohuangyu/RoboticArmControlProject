...
LK4005_Motor_Handle_t LK4005_Motor_Handle[LK4005_Motor_Number];
...
{
    if(LK4005_Motor_Handle.Motor_Torque_Current >= -33.0f && LK4005_Motor_Handle.Motor_Torque_Current <= 33.0f)
    {
        uint8_t FDCAN_Send_Temp[LK4005_Motor_FDCAN_Length] = {0};
        int16_t Torque_Temp = (int16_t)(LK4005_Motor_Handle.Motor_Torque_Current * Torque_Conersion_Ratio);
...
void LK4005_Motor_Response_Data_Explain(FDCAN_HandleTypeDef *hfdcan, FDCAN_RxHeaderTypeDef FDCAN_Rx_Head_Temp, uint8_t *FDCAN_Rx_Data_Temp, LK4005_Motor_Handle_t *LK4005_Motor_Handle)
{
    if (hfdcan == LK4005_Motor_Handle->Motor_FDCAN_Handle)
    {
        if (FDCAN_Rx_Head_Temp.Identifier == LK4005_Motor_Handle->Motor_ID && FDCAN_Rx_Head_Temp.DataLength == LK4005_Motor_FDCAN_Length)
        {
            ...
            {
                LK4005_Motor_Handle->Motor_Position = ((float)(((uint32_t)FDCAN_Rx_Data_Temp[7] << 24) | ((uint32_t)FDCAN_Rx_Data_Temp[6] << 16) | ((uint32_t)FDCAN_Rx_Data_Temp[5] << 8) | ((uint32_t)FDCAN_Rx_Data_Temp[4])) * Position_Conversion_Ratio / Reduction_Ratio) * PI / 180.0f;
            }
        }
    }
}
